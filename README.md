...
The fraud labels in this project were generated using deterministic rules applied to a synthetic dataset — not sourced from real fraud investigations. This means the model is largely learning the rules back rather than discovering hidden fraud patterns from ground truth data.

The PR-AUC of 0.9287 reflects how well the model recovers a clean, rule-based signal — not how it would perform on real-world messy fraud data where the boundary between fraud and legitimate behaviour is far less defined.

This is an intentional design choice. The dataset was a raw log parsing challenge with no ground truth labels. The goal was to demonstrate the ability to build a complete end-to-end ML system — not to claim production readiness for a live fraud environment.

**What this project actually demonstrates:**

- Parsing and cleaning unstructured multi-format log data at scale
- Designing a feature engineering pipeline for transactional data
- Handling extreme class imbalance correctly (SMOTE in CV, ADASYN on refit)
- Multi-model comparison with proper cross-validation and leakage prevention
- Threshold tuning, SHAP explainability, and business impact quantification
- Production serving via FastAPI with a live Streamlit dashboard

### What Would Change With Real Data

If this system were deployed against real transaction data with ground truth fraud labels from a case management system, the following would change:

**Labels** — Replace simulated rules with confirmed fraud cases from dispute resolution and fraud investigation teams. Labels would be noisier and harder to learn from, but genuinely meaningful.

**Features** — Add network features (is this device linked to other fraud cases?), merchant features (fraud rate at this merchant category), and behavioural sequences (unusual login patterns before the transaction).

**Velocity** — With millions of daily transactions per user cohort, velocity features would fire meaningfully. The 1-day and 7-day rolling windows would become strong signals rather than near-zero contributors.

**Model validation** — Use time-based splits rather than random splits. Train on January-October, validate on November, test on December. This reflects real deployment where the model is always predicting the future from the past.

**Monitoring** — Add data drift detection and model performance monitoring. Fraud patterns evolve — fraudsters adapt to detection systems. A production model needs retraining triggers and performance dashboards.

**Threshold** — In production the threshold is a business decision involving fraud team capacity, customer friction tolerance, and regulatory requirements — not just F1 maximisation on a validation set.
